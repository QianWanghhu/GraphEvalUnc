...
bounds <- read.csv("condition uc bounds_zero_for_non_lab.csv")
conditions <- unique(bounds$Condition)

nalookup <- read.csv("na lookup_zero_for_non_lab.csv")
...
set.seed(123)
samples <- matrix(runif(609*10, min = 0.96, max = 1.04), nrow = 609, ncol = 10)
# Calculate base realisation without noise added.
noxuncup$concuncbound = 0
din_copy <- dinraw
index <- 1:101
columns <- 2009:2018
...
dinlabunc[1, ] <- samplefreqfun(flowdf = flow, concdf = dinraw, seconds = yearlyseconds, minconc = MinDIN,
...
for (ii in 1:ncol(samples)) {
  din_copy$DIN <- dinraw$DIN * samples[,ii]
  dinlabunc[ii + 1, ] <- samplefreqfun(flowdf = flow, concdf = din_copy, seconds = yearlyseconds, minconc = MinDIN,
                           loadfun = "linear",  biasfun = 1, randomfun = "randomyear", randomseedprop = 1.0,
                           randomseeds = 10, randomtimes = 1, flowunc = flowuncup,
                           concunc = noxuncup, covar = FALSE, boundtype = "upper")
}
# write.csv(dinlabunc, 'output/resample_freq100_new_assumption/dinlabunc_nocovar_random_sample_conc.csv')
...
# 
# write.csv(dinmixlow, 'output/resample_freq100_new_assumption/dinmixlow_covar_100.csv')
# 
# 
...
#                            concunc = noxuncup, covar = FALSE, boundtype = "upper")
# 
# write.csv(dinmixup, 'output/resample_freq100_new_assumption/dinmixup_covar_100.csv')
