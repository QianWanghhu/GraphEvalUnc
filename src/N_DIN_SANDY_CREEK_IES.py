...
class spot_setup(object):
    ...
    def simulation(self,parameters_vector, model_fn, model_dataframes, new_output_fn): #Body of the class where we call and run OW
        ...
        din =  new_results.time_series('InstreamDissolvedNutrientDecay','incomingMassUpstream', 'catchment',constituent='N_DIN')[SC] #Query the results
...
MODEL_NAME= 'MW_RC_13_WQT_126001A_Qian' #The name of the actual .h5 file. Can be the same as the model name we call in python
...
parameters = pd.read_csv('output/work_100_0.005_4_unbased/parameter_absolute_value.csv', index_col='real_name')
for index, row in parameters.iterrows():
    ...
    flow_load.to_csv(f'output/DailyData_work_100_0.005_4_unbase/RUN_{index}.csv')
...
flow_load.columns = ['Q (m3/s)', 'Load (t)']
flow_load.to_csv('output/DailyData_work_100_0.005_4_unbase/RunDefaultPar.csv')